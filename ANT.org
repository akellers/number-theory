...
** Ideale in Produkten von Ringen
   ...
   Anmerkung: Das Lemma lässt sich auf offensichtliche Weise erweitern auf endliche Produkte von
   Ringen: Die Ideale von $A_1 \times \cdots \times A_m$ sind der Gestalt $\mathfrak{a}_1 \times
   \cdots \times \mathfrak{a}_m$ mit Idealen $\mathfrak{a}_i$ in $A_i$ und $\mathfrak{a}_1 \times
   \cdots \times \mathfrak{a}_m$ ist ein Primideal genau dann, wenn es genau ein $j$ gibt mit
   $\mathfrak{a}_j$ ist Primideal in $A_j$ und $\mathfrak{a}_i = A_i$ für alle $i \neq j$.
...
** Noethersche Ringe
   ...
   *Beweis*: Von 1) nach 2): $\mathfrak{a} = \bigcup \mathfrak{a}_i$ ist ein Ideal und damit endlich erzeugt. Sei
   ...
   Ein berühmters Theorem von Jilbert sagt aus, dass $k[X_1, \ldots, X_n]$ noethersch
   ist. Praktisch alle Ringe in der algebraischen Zahlentheorie sind noethersch, aber nicht alle
   Ringe sind noethersch, wie z.B. der Ring $k[X_1, \ldots, X_n, \ldots]$ der Polynome in unendlich
   vielen Variablen.
